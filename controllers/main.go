...
func (controller *MainController) APIAddress(c web.C, r *http.Request) ([]string, codes.Code, string, error) {
	...
	poolValidateAddress, err := controller.ControllerParams.StakepooldServers.ValidateAddress(pooladdress)
	if err != nil {
	...
	createMultiSig, err := controller.ControllerParams.StakepooldServers.CreateMultisig([]string{poolPubKeyAddr, userPubKeyAddr})
	if err != nil {
		return nil, codes.Unavailable, "system error", errors.New("unable to process wallet commands")
	}
	...
	importedHeight, err = controller.ControllerParams.StakepooldServers.ImportScript(serializedScript)
	if err != nil {
		return nil, codes.Unavailable, "system error", errors.New("unable to process wallet commands")
	}
...
func (controller *MainController) APIPurchaseInfo(c web.C,
	...
	purchaseInfo := &poolapi.PurchaseInfo{
		...
		PoolFees:      controller.ControllerParams.PoolFees,
...
func (controller *MainController) APIStats(c web.C,
	...
	gsi, err := controller.ControllerParams.StakepooldServers.GetStakeInfo()
	if err != nil {
	...
	if controller.ControllerParams.ClosePool {
		poolStatus = "Closed"
	} else {
		poolStatus = "Open"
	}
	...
	stats := &poolapi.Stats{
		...
		APIVersionsSupported: controller.ControllerParams.APIVersionsSupported,
		...
		PoolEmail:            controller.ControllerParams.PoolEmail,
		PoolFees:             controller.ControllerParams.PoolFees,
...
func (controller *MainController) isAdmin(c web.C, r *http.Request) (bool, error) {
	remoteIP := getClientIP(r, controller.ControllerParams.RealIPHeader)
	...
	if !stringSliceContains(controller.ControllerParams.AdminIPs, remoteIP) {
		return false, fmt.Errorf("%s request from %s "+
			"userid %s failed AdminIPs check", r.URL, remoteIP, uidstr)
	}

	if !stringSliceContains(controller.ControllerParams.AdminUserIDs, uidstr) {
		return false, fmt.Errorf("%s request from %s "+
...
func (controller *MainController) StakepooldUpdateTickets(dbMap *gorp.DbMap) error {
	...
	err = controller.ControllerParams.StakepooldServers.SetAddedLowFeeTickets(votableLowFeeTickets)
	if err != nil {
...
func (controller *MainController) StakepooldUpdateUsers(dbMap *gorp.DbMap) error {
	...
	err = controller.ControllerParams.StakepooldServers.SetUserVotingPrefs(allUsers)
	if err != nil {
...
func (controller *MainController) FeeAddressForUserID(uid int) (dcrutil.Address,
	...
	acctKey := controller.ControllerParams.FeeXpub
	index := uint32(uid)
...
func (controller *MainController) TicketAddressForUserID(uid int) (dcrutil.Address,
	...
	acctKey := controller.ControllerParams.VotingXpub
	index := uint32(uid)
...
func (controller *MainController) RPCSync(dbMap *gorp.DbMap) error {
	...
	err = controller.ControllerParams.StakepooldServers.SyncAll(multisigScripts, MaxUsers)
	return err
}
...
func (controller *MainController) Address(c web.C, r *http.Request) (string, int) {
	...
	c.Env["PoolFees"] = controller.ControllerParams.PoolFees
	...
	if user.APIToken == "" {
		token, err := models.SetUserAPIToken(dbMap, controller.ControllerParams.APISecret,
			controller.ControllerParams.BaseURL, user.Id)
		if err != nil {
	...
	c.Env["Designation"] = controller.ControllerParams.Designation
...
func (controller *MainController) AdminStatus(c web.C, r *http.Request) (string, int) {
	...
	backendStatus := controller.ControllerParams.StakepooldServers.BackendStatus()
...
func (controller *MainController) AdminTickets(c web.C, r *http.Request) (string, int) {
	...
	ignoredLowFeeTickets, err := controller.ControllerParams.StakepooldServers.GetIgnoredLowFeeTickets()
	if err != nil {
...
func (controller *MainController) Index(c web.C, r *http.Request) (string, int) {
	if controller.ControllerParams.ClosePool {
		c.Env["IsClosed"] = true
		c.Env["ClosePoolMsg"] = controller.ControllerParams.ClosePoolMsg
	}
	c.Env["Network"] = controller.params.Name
	c.Env["PoolEmail"] = controller.ControllerParams.PoolEmail
	c.Env["PoolFees"] = controller.ControllerParams.PoolFees
	c.Env["CustomDescription"] = controller.ControllerParams.Description
	c.Env["PoolLink"] = controller.ControllerParams.PoolLink
...
func (controller *MainController) PasswordResetPost(c web.C, r *http.Request) (string, int) {
	...
	if err == nil {
		...
		err := controller.ControllerParams.EmailSender.PasswordChangeRequest(user.Email, remoteIP, controller.ControllerParams.BaseURL, token.String())
		if err != nil {
...
func (controller *MainController) SettingsPost(c web.C, r *http.Request) (string, int) {
	...
	if updateEmail == "true" {
		...
		err = controller.ControllerParams.EmailSender.EmailChangeVerification(controller.ControllerParams.BaseURL, user.Email, newEmail, remoteIP, token.String())
		if err != nil {
		...
		err = controller.ControllerParams.EmailSender.EmailChangeNotification(controller.ControllerParams.BaseURL, user.Email, newEmail, remoteIP)
		// inform the user.
		if err != nil {
		...
		err = controller.ControllerParams.EmailSender.PasswordChangeConfirm(user.Email, controller.ControllerParams.BaseURL, remoteIP)
		if err != nil {
...
func (controller *MainController) RegisterPost(c web.C, r *http.Request) (string, int) {
	...
	err = controller.ControllerParams.EmailSender.Registration(email, controller.ControllerParams.BaseURL, remoteIP, token.String())
	if err != nil {
...
func (controller *MainController) Tickets(c web.C, r *http.Request) (string, int) {
	...
	if len(ticketInfoVoted) > controller.ControllerParams.MaxVotedTickets {
		ticketInfoVoted = ticketInfoVoted[0:controller.ControllerParams.MaxVotedTickets]
	}
	...
	c.Env["TicketsVotedMaxDisplay"] = controller.ControllerParams.MaxVotedTickets
